...
function ChatPage() {
    ...
    const showGreeting = (messageContent) => {
        setMessages(prevMessages => [...prevMessages, { id: Date.now(), content: messageContent }]);
    };
...
<div tw="flex min-h-full flex-1 flex-col justify-center px-10 py-10 lg:px-10">
        ...
        <div tw="flex-grow">
        <div tw="flex items-center justify-between py-2">            <CreateRoom setRoom={setRoom} />
                    ...
                    <table id="conversation" tw="table-auto w-full border-collapse">
                        <thead>
                            ...
                            <th tw="border px-4 py-2 text-gray-900 bg-sky-200 rounded">
                                <img src="/logo.png" alt="Logo" tw="w-6 h-auto mr-2 inline-block" />
                                Chat
                            </th>
                            </tr>
                        </thead>
                        <tbody id="greetings">
                            {messages.map((message, index) => (
                                <tr key={`message.sender${message.sender}_${index}`}>
                                    <td tw="border px-4 py-2 text-gray-900">{message.message}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
        ...
        <div tw="fixed bottom-0 left-0 right-0 bg-white p-4 border-t">
            <form tw="container mx-auto flex items-center" onSubmit={e => e.preventDefault()}>
                <input type="text" id="message" tw="px-2 py-1 border rounded mr-2 flex-grow text-gray-900" placeholder="메세지" ref={inputRef} value={currentMessage} onChange={handleMessageChange} />
                <button id="send" type="button" tw="bg-sky-500 hover:bg-sky-600 px-2 py-1 text-gray-900 rounded" onClick={handleSendMessage} >전송하기</button>
